import { PrismaClient, Role, Severity, AlertStatus, AlertCategory, ResourceStatus, ResourceHorizon, AlertMessageType } from '@prisma/client';
...
async function seedAlerty(
  ...
  // realną organizationId do wykorzystania przez authz/R1/R6/R11. Kategoria
  // (AlertCategory) dobrana ręcznie do treści każdego zdarzenia — bez tego
  // pola wszystkie padały na domyślne GENERAL, przez co pinezki na mapie w
  // trybie "Kategorie" były jednolicie szare zamiast kolorowych wg typu.
  const alerty: Array<{
    title: string;
    description: string;
    severity: Severity;
    ...
    { title: 'Podtopienia posesji przy ul. Rzecznej', description: 'Potok Dębianka wystąpił z koryta po nawalnych opadach deszczu, woda wdarła się na teren kilku posesji prywatnych.', severity: 'CRITICAL', status: 'ACTIVE', category: 'HYDROLOGICAL', location: 'ul. Rzeczna', ...spreadAround(0, 19, 2.4), expiresAt: godziny(48) },
    { title: 'Awaria sieci wodociągowej – os. Poligon', description: 'Przerwa w dostawie wody pitnej, trwa naprawa magistrali wodociągowej.', severity: 'MEDIUM', status: 'IN_PROGRESS', category: 'INFRASTRUCTURE', location: 'os. Poligon', ...spreadAround(1, 19, 2.4), expiresAt: godziny(20) },
    { title: 'Zerwany dach hali sportowej', description: 'Silny wiatr uszkodził pokrycie dachowe, teren zabezpieczony przez straż pożarną.', severity: 'HIGH', status: 'ACTIVE', category: 'INFRASTRUCTURE', location: 'ul. Sportowa 3', ...spreadAround(2, 19, 2.4), expiresAt: godziny(24) },
    { title: 'Pożar poszycia leśnego na obrzeżach Poligonu OSPWL', description: 'Pożar traw i poszycia leśnego, jednostki straży pożarnej prowadzą działania gaśnicze na miejscu.', severity: 'CRITICAL', status: 'ACTIVE', category: 'FIRE', location: 'Poligon OSPWL Nowa Dęba', latitude: 50.3980, longitude: 21.7180, expiresAt: godziny(12) },
    { title: 'Uszkodzona linia energetyczna – Osiedle Zachodnie', description: 'Zerwana linia napowietrzna po silnym wietrze, wstrzymane dostawy prądu w części gminy.', severity: 'HIGH', status: 'IN_PROGRESS', category: 'INFRASTRUCTURE', location: 'Osiedle Zachodnie', ...spreadAround(3, 19, 2.4), expiresAt: godziny(16) },
    { title: 'Zwalone drzewo na drodze powiatowej', description: 'Droga częściowo zablokowana, utrudniony przejazd w kierunku Rozalina.', severity: 'LOW', status: 'RESOLVED', category: 'ROAD', location: 'Droga powiatowa Rozalin-Jadachy', latitude: 50.4260, longitude: 21.7610, expiresAt: godziny(-2) },
    { title: 'Ostrzeżenie IMGW – silny wiatr', description: 'Ostrzeżenie 2. stopnia przed silnym wiatrem do jutra rana, możliwe dalsze uszkodzenia dachów i linii energetycznych.', severity: 'MEDIUM', status: 'ACTIVE', category: 'HYDROLOGICAL', location: 'cała gmina', latitude: 50.4166, longitude: 21.7500, expiresAt: godziny(18) },
    { title: 'Ćwiczenia ewakuacyjne szkoły podstawowej', description: 'Planowe ćwiczenia służb ratowniczych, brak realnego zagrożenia.', severity: 'LOW', status: 'CANCELLED', category: 'GENERAL', location: 'Szkoła Podstawowa nr 1', ...spreadAround(4, 19, 2.4), expiresAt: godziny(-24) },
    { title: 'Osunięcie skarpy przy drodze wojewódzkiej', description: 'Częściowe osunięcie skarpy, droga zwężona do jednego pasa ruchu.', severity: 'MEDIUM', status: 'IN_PROGRESS', category: 'ROAD', location: 'Droga wojewódzka 985', latitude: 50.4090, longitude: 21.7620, expiresAt: godziny(36) },
    { title: 'Podtopienie gospodarstw rolnych – Cygany', description: 'Woda z okolicznych rowów melioracyjnych zalała pola i zabudowania gospodarcze w sołectwie Cygany.', severity: 'HIGH', status: 'ACTIVE', category: 'HYDROLOGICAL', location: 'Sołectwo Cygany', latitude: 50.3860, longitude: 21.7460, expiresAt: godziny(30) },
    { title: 'Wylew rzeki Wisły – Chmielów', description: 'Wysoki stan wody spowodował zalanie terenów nadrzecznych w Chmielowie, ewakuowano kilka gospodarstw.', severity: 'CRITICAL', status: 'ACTIVE', category: 'HYDROLOGICAL', location: 'Chmielów, ul. Nadwiślańska', latitude: 50.4510, longitude: 21.6890, expiresAt: godziny(60) },
    { title: 'Pożar stodoły – Alfredówka', description: 'Pożar zabudowań gospodarczych, ogień zagraża sąsiednim budynkom.', severity: 'HIGH', status: 'ACTIVE', category: 'FIRE', location: 'Alfredówka', latitude: 50.4330, longitude: 21.7720, expiresAt: godziny(10) },
    { title: 'Awaria transformatora – Tarnowska Wola', description: 'Przerwa w dostawie prądu dla części miejscowości, trwa naprawa stacji transformatorowej.', severity: 'MEDIUM', status: 'IN_PROGRESS', category: 'INFRASTRUCTURE', location: 'Tarnowska Wola', latitude: 50.4680, longitude: 21.7010, expiresAt: godziny(14) },
    { title: 'Zerwany most na potoku – Poręby Dębskie', description: 'Wezbrana woda uszkodziła konstrukcję mostu, wstrzymano ruch pojazdów.', severity: 'HIGH', status: 'ACTIVE', category: 'ROAD', location: 'Poręby Dębskie', latitude: 50.4400, longitude: 21.7900, expiresAt: godziny(40) },
    { title: 'Uszkodzona kamienica po nawałnicy – ul. Kościuszki', description: 'Silny wiatr zerwał część elewacji budynku wielorodzinnego, konieczne zabezpieczenie terenu.', severity: 'MEDIUM', status: 'ACTIVE', category: 'INFRASTRUCTURE', location: 'ul. Kościuszki', ...spreadAround(5, 19, 2.4), expiresAt: godziny(22) },
    { title: 'Zalanie piwnic po ulewie – Rynek', description: 'Intensywne opady spowodowały zalanie piwnic budynków przy Rynku.', severity: 'MEDIUM', status: 'ACTIVE', category: 'HYDROLOGICAL', location: 'Rynek Nowa Dęba', ...spreadAround(6, 19, 2.4), expiresAt: godziny(15) },
    { title: 'Zerwana trakcja kolejowa – Dworzec PKP', description: 'Uszkodzenie sieci trakcyjnej wstrzymało ruch pociągów przez Nową Dębę.', severity: 'HIGH', status: 'IN_PROGRESS', category: 'INFRASTRUCTURE', location: 'Dworzec PKP Nowa Dęba', ...spreadAround(7, 19, 2.4), expiresAt: godziny(9) },
    { title: 'Pożar lasu komunalnego – Osiedle Podleśne', description: 'Pożar lasu w pobliżu zabudowań mieszkalnych, prowadzone działania gaśnicze z powietrza i ziemi.', severity: 'CRITICAL', status: 'ACTIVE', category: 'FIRE', location: 'Osiedle Podleśne', ...spreadAround(8, 19, 2.4), expiresAt: godziny(8) },
    { title: 'Powalone drzewa blokujące dojazd – ul. Leśna', description: 'Kilka powalonych drzew blokuje jedyny dojazd do części osiedla.', severity: 'LOW', status: 'RESOLVED', category: 'ROAD', location: 'ul. Leśna', ...spreadAround(9, 19, 2.4), expiresAt: godziny(-6) },
    { title: 'Awaria zapory ziemnej – Zalew w Chmielowie', description: 'Stwierdzono nieszczelność zapory, istnieje ryzyko zalania terenów poniżej zbiornika.', severity: 'CRITICAL', status: 'ACTIVE', category: 'HYDROLOGICAL', location: 'Zalew w Chmielowie', latitude: 50.4560, longitude: 21.6840, expiresAt: godziny(50) },
    { title: 'Zalanie ulicy po awarii kanalizacji – ul. Kwiatowa', description: 'Awaria kolektora sanitarnego spowodowała zalanie jezdni i chodników.', severity: 'MEDIUM', status: 'ACTIVE', category: 'INFRASTRUCTURE', location: 'ul. Kwiatowa', ...spreadAround(10, 19, 2.4), expiresAt: godziny(13) },
    { title: 'Osunięcie ziemi po ulewnych deszczach – ul. Polna', description: 'Nasiąknięte zbocze osunęło się częściowo na jezdnię, droga zwężona.', severity: 'HIGH', status: 'ACTIVE', category: 'ROAD', location: 'ul. Polna', ...spreadAround(11, 19, 2.4), expiresAt: godziny(28) },
    { title: 'Zwalone ogrodzenie i drzewa – cmentarz komunalny', description: 'Silny wiatr powalił część ogrodzenia i kilka drzew na terenie cmentarza.', severity: 'LOW', status: 'RESOLVED', category: 'GENERAL', location: 'Cmentarz komunalny', ...spreadAround(12, 19, 2.4), expiresAt: godziny(-10) },
    { title: 'Awaria ogrzewania w mroźną noc – Przedszkole nr 3', description: 'Awaria kotłowni pozostawiła placówkę bez ogrzewania, konieczne tymczasowe grzejniki.', severity: 'MEDIUM', status: 'ACTIVE', category: 'INFRASTRUCTURE', location: 'Przedszkole nr 3', ...spreadAround(13, 19, 2.4), expiresAt: godziny(19) },
    { title: 'Zalanie sali widowiskowej – Dom Kultury', description: 'Nieszczelny dach po ulewie spowodował zalanie sali i sprzętu nagłośnieniowego.', severity: 'LOW', status: 'IN_PROGRESS', category: 'INFRASTRUCTURE', location: 'Dom Kultury', ...spreadAround(14, 19, 2.4), expiresAt: godziny(11) },
    { title: 'Uszkodzone trybuny po wichurze – Stadion Miejski', description: 'Silny wiatr zerwał zadaszenie części trybun stadionu.', severity: 'MEDIUM', status: 'ACTIVE', category: 'INFRASTRUCTURE', location: 'Stadion Miejski', ...spreadAround(15, 19, 2.4), expiresAt: godziny(17) },
    { title: 'Pożar budynku wielorodzinnego – ul. Krótka', description: 'Pożar w jednym z mieszkań rozprzestrzenił się na klatkę schodową, trwa ewakuacja mieszkańców.', severity: 'CRITICAL', status: 'ACTIVE', category: 'FIRE', location: 'ul. Krótka', ...spreadAround(16, 19, 2.4), expiresAt: godziny(6) },
    { title: 'Zerwana sieć telekomunikacyjna – ul. Wschodnia', description: 'Uszkodzenie linii światłowodowej pozbawiło część mieszkańców łączności internetowej.', severity: 'LOW', status: 'CANCELLED', category: 'INFRASTRUCTURE', location: 'ul. Wschodnia', ...spreadAround(17, 19, 2.4), expiresAt: godziny(-16) },
    { title: 'Skażenie ujęcia wody pitnej – Osiedle Poligon II', description: 'Podejrzenie zanieczyszczenia lokalnego ujęcia wody, do odwołania obowiązuje zakaz picia wody z kranu.', severity: 'CRITICAL', status: 'ACTIVE', category: 'HYDROLOGICAL', location: 'Osiedle Poligon II', ...spreadAround(18, 19, 2.4), expiresAt: godziny(44) },
    { title: 'Podtopienie dróg dojazdowych – Jadachy', description: 'Wezbrane rowy przydrożne zalały odcinki dróg gminnych, utrudniony dojazd do sołectwa.', severity: 'HIGH', status: 'ACTIVE', category: 'ROAD', location: 'Jadachy', latitude: 50.4340, longitude: 21.7650, expiresAt: godziny(26) },
    { title: 'Zerwana linia energetyczna po burzy – Rozalin', description: 'Upadłe drzewo zerwało linię energetyczną, kilkanaście gospodarstw w sołectwie zostało bez prądu.', severity: 'MEDIUM', status: 'ACTIVE', category: 'INFRASTRUCTURE', location: 'Rozalin', latitude: 50.4400, longitude: 21.7550, expiresAt: godziny(20) },
  ];
