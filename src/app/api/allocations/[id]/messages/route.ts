...
async function findAllocationWithAlert(id: string) {
  return prisma.resourceAllocation.findUnique({
    where: { id },
    include: { alert: { select: { id: true, organizationId: true } } },
  });
}

// Narrow, "working" chat between the two organizations actually involved in
// ONE allocation — donor, recipient, or ADMIN (нюанс #8,
// docs/are-you-familiar-with-tidy-blum.md). A COORDINATOR from an unrelated
// third organization has no business reading this: it's deliberately
// narrower than the broad, alert-wide AlertMessage forum (R12/Крок 50), which
// every involved org can see.
function canAccessAllocationMessages(
  allocation: { donorOrgId: string; alert: { organizationId: string | null } },
  user: { role: string; organizationId?: string | null }
): boolean {
  return user.role === 'ADMIN' || isAllocationDonor(allocation, user) || isAllocationRecipient(allocation, user);
}
